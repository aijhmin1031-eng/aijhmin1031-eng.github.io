...
> 처음 이어받았다면 **`docs/HANDOFF.md`를 먼저 읽어라** — 사이트 구조 지도, 도구 사용법,
> 실제 사고 이력, 자동화 루틴, Drive·계정 정보, 진행 상황이 전부 정리돼 있다.
> 이 파일은 그중 "매일 하는 작업"의 절차서다.
...
- **푸시 전 반드시 `python3 scripts/check_build.py` 실행.** YAML 한 줄이 깨지면
  Jekyll 빌드가 통째로 실패하고 **사이트 전체가 옛 버전에 멈춘다**(2026-08-21 실제 사고:
  `desc: "사랑해", "..."` — 큰따옴표로 시작한 값 뒤에 쉼표가 와서 빌드 중단, /shop/ 404).
  한국어 설명에 따옴표가 들어가면 값 전체를 작은따옴표로 감쌀 것: `desc: '"사랑해" ...'`
- 배포 후에는 새 페이지 URL을 curl로 200 확인할 것. 200이 아니면 빌드 실패를 의심한다.
- 새 페이지/글 배포 후에는 `curl`로 사이트맵을 확인해 **의도한 URL만** 실려 있는지 점검한다.

## 디자인 원칙 (반드시 지킬 것)

- **막대(선)로 강조하지 않는다.** 세로 막대(border-left)·가로 룰·밑줄 강조 전부 금지.
  위계는 **크기·굵기·색·여백**으로만 만든다. 이미지 조판에서도 동일
  (`d.rectangle`로 구분선 긋지 말 것).
- 한글 조판: `word-break: keep-all` 유지 — 단어 중간 줄바꿈 방지가 가독성의 핵심.
  본문 17px / 행간 1.85, 제목 자간 -.022em.
- 제목 서체 고운바탕, 본문 Noto Sans KR. 시스템 폰트로 대체하지 말 것.
